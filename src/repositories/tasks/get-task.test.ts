...
import { GetTasksRepository } from "./get-task";

describe("GetTaskRepository", () => {
  const makeSut = () => {
    const sut = new GetTasksRepository();
    const createTaskRepository = new CreateTaskRepository();

    return { sut, createTaskRepository };
  };
  ...
  it("should return tasks with success", async () => {
    const user = await createUser();
    const { sut, createTaskRepository } = makeSut();
    await createTaskRepository.execute({
      ...task,
      user_id: user.id,
    });
